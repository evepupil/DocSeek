...
import { prepareTokenBatch } from "../../src/embedding/token-inputs.js";

describe("embedding token inputs", () => {
  ...
  it("rejects missing special token ids", () => {
    expect(() => prepareTokenBatch({ inputIds: [[1, 2]] }, 8, Number.NaN, 2)).toThrow(
      "valid padding and separator tokens",
    );
  });
});
